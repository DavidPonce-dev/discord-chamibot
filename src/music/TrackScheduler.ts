...
export class TrackScheduler {
  ...
  private async findRadioTrack(target: 'queue' | 'next'): Promise<Track | null> {
    const searchTitle = this.radioBaseTitle ?? this.lastTrackTitle
    if (!searchTitle) return null
  ...
  private async populateRadioQueue(): Promise<void> {
    if (!this.autoplay || this.radioQueue.length > 0) return

    const track = await this.findRadioTrack('queue')
    if (!track) return
  ...
  async reshuffleRadio(): Promise<Track | null> {
    this.radioNext = null
    const track = await this.findRadioTrack('next')
    if (track) {
